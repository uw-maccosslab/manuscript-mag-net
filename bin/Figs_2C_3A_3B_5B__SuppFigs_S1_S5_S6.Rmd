---
title: "Figures 2C, 3A, 3B, 5B, and Supplementary Figures S1, S5 and S6"
...
Data from three separate experiments to generate 6 figure panels were assessed in this R markdown:
...
* Figures (as .svg files): 
  ...
  - Supplementary Figure S1 (Labeled volcano plot)
  
  - Supplementary Figure S5 (Individual protein abundance panels - CD9, NCAM1 and APOB as separate files )
  
  - Supplementary Figure S6 (Freeze thaw ratio relative to 1 freeze thaw cycle)
...
knitr::opts_knit$set(echo = TRUE, root.dir = 'C:\\Users\\Kristine\\Desktop\\Research\\05_PlasmaECVs\\MagNet_ChrisWu\\script')
...
# Data to generate enrichment/depletion plots and volcano plots
        AllGroups_FC <- read.csv("input\\group-fc-Beads-SAXN-Particles vs HydN-Plasma-EncV2score-QuantReport_2023-03-05_13-29-41.csv", 
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",")    
...
# Matched-Matrix Calibration Curve data
    # Total area fragment (No normalization)
        MM_peptides <- read.csv("input\\MMCC_EV_Eclipse_Peptide_Long_TAF_NoNorm.csv",
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",")
    # Protein abundance
       MM_proteins <- read.csv("input\\MMCC_EV_Eclipse_Protein_Long_ProtAb_NoNorm.csv",
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",")            
    # Figures of Merit calculated using protein abundance.
        ...
        MM_prot_FOM <- read.csv("input\\MMCC_EV_Eclipse_figuresofmerit_proteins_NoNorm.csv",
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",") %>%
    ...
    # Metadata for MMCC
        MM_meta <- read.csv("input\\MMCC_EV_Eclipse_files_meta.csv", 
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",")     
...
# Freeze-thaw data
    # Normalized area
        FT_peptides <- read.csv("input\\FT_TICnormalized_LongForm_Peptide_NormArea_Long.csv",
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",")    
    # Freeze-thaw metadata
        FT_meta <- read.csv("input\\FT_LongForm_Reps_Meta.csv", 
                              header = TRUE,
                              na.strings="#N/A",
                              sep = ",")    
...
write.csv(Simplified_FC, paste(basename,enrichdeplet,"FCtype.csv", sep=""), quote=FALSE, row.names=FALSE)
...
write.csv(percent_depletion, paste(basename,enrichdeplet,"depleted_percents.csv", sep=""), quote=FALSE, row.names=FALSE)
...
write.csv(percent_enrichment, paste(basename,enrichdeplet,"enriched_percents.csv", sep=""), quote=FALSE, row.names=FALSE)
...
# Export plot as svg
ggsave(file = paste(basename,enrichdeplet,"Figure2C.svg", sep = ""), plot=volcanoplot, width=3.1, height=2.07)
...
# Export plot as svg
ggsave(file = paste(basename,enrichdeplet,"Supplementary_Figure_S1.svg", sep = ""), plot=volcanoplotlabeled, width=newwidth, height=8)
...
# Merging the foldchange data with the "organizer" file to plot proteins of interest.
  ...
  #Generate entries in the dataframe for spacers between exosome, microvesicle, and apoptotic body/cell debris on x-axis.
        spacer_setup <- Simplified_FC_graph[-c(1:40), ]
...
# Export plot as svg
ggsave(file = paste(basename,enrichdeplet,"Figure_3A.svg", sep=""), plot=barplot_marker_GC_FC_marker, width=8, height=5)
...
# Export plot as svg
ggsave(file = paste(basename,enrichdeplet,"Figure_3B.svg", sep=""), plot=barplot_plasma_GC_FC, width=8, height=5)
...
write.csv(MM_pep_reps, paste(basename,matrixinfo,"peptide_replicates.csv", sep=""), quote=FALSE, row.names=FALSE)
...
write.csv(MM_pep_summ, paste(basename,matrixinfo,"peptide_Avg_Summary.csv", sep=""), quote=FALSE, row.names=FALSE)
...
write.csv(MM_prot_reps, paste(basename,matrixinfo,"protein_replicates.csv", sep=""), quote=FALSE, row.names=FALSE)
...
write.csv(MM_prot_summ, paste(basename,matrixinfo,"protein_Avg_Summary.csv", sep=""), quote=FALSE, row.names=FALSE)
...
ggsave(file = paste(basename,matrixinfo,"Pep",pep_BH,"Figure_5B.svg", sep = ""), plot=plot_pep_boxviol, width=5, height=3.75)
...
ggsave(file = paste(basename,matrixinfo,"Supplementary_Figure_S5_","protein_CD9_","P21926.svg", sep = ""), plot=MM_plot_prot_cd9, width=5, height=3.5)
...
ggsave(file = paste(basename,matrixinfo,"Supplementary_Figure_S5_","protein_NCAM1_","P13591.svg", sep = ""), plot=MM_plot_prot_ncam1, width=5, height=3.5)
...
ggsave(file = paste(basename,matrixinfo,"Supplementary_Figure_S5_","protein_APOB_","P04114.svg", sep = ""), plot=MM_plot_prot_apob, width=5, height=3.5)
...
write.csv(FT_pep_reps, paste(basename,freezeinfo,"ratios_replicates.csv", sep=""), quote=FALSE, row.names=FALSE)
...
write.csv(FT_pep_summary_byFT, paste(basename,freezeinfo,"Ratios_summary_ByFTcycle.csv", sep=""), quote=FALSE, row.names=FALSE)

```

$~$

```{r FT_violhalf_FT_BHV_settings}
# Set aesthetic components of volcano plot to be generated in next section.
...
```{r FT_BHV_plot, warning=FALSE, message=FALSE, echo=FALSE}
...
# Saving plot as an .svg format. 
ggsave(file = paste(basename,freezeinfo,"BHV_AllPeptides_Supplementary_Figure_S6.svg", sep = ""), plot=plot_FT_pep_boxviol, width=5, height=3.75)
